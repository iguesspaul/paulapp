...
def count_bets_today(session_start_ts):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM simulated_bets WHERE timestamp >= ?", (session_start_ts,)
        )
...
def write_session_log(session_start, session_end, new_bets, total_bets, summary):
    ...
    entry = (
        f"\n## Session — {session_start.strftime('%Y-%m-%d %H:%M timezone.utc')}\n"
    ...
    entry += "- **Report**: [PERFORMANCE_SEGMENTATION.md](PERFORMANCE_SEGMENTATION.md)\n"
...
async def run():
    ...
    print(f"  {datetime.now(UTC).strftime('%Y-%m-%d %H:%M timezone.utc')}")
    print("=" * 50)
    ...
    print("  Report          : PERFORMANCE_SEGMENTATION.md")
    print("  Session log     : SESSION_LOG.md")
    print(f"{'=' * 50}\n")
