...
def generate_report():
    ...
    lines = [
        ...
        f"\n_Generated: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M timezone.utc')}_\n",
        "---",
        "## Bankroll Overview",
        "| Metric | Value |",
        "|---|---|",
    ...
    if sessions:
        ...
        for i, s in enumerate(sessions, 1):
            ...
            lines.append(
                f"| {i} | {s_type} | {bets} | {stake_str} | {ev_str} | {pnl_str} | {bal_str} |"
            )
        lines.append("")
    ...
    if not rows:
        ...
        lines.append(
            "| Category | Volume | Settled | Win Rate | Expected WR | Model Accuracy | Yield (ROI) | Stake Exposure | P&L ($) |"
        )
        lines.append("|---|---|---|---|---|---|---|---|---|")
        ...
        for row in rows:
            ...
            model_str = (
                f"**{model_acc * 100:+.1f}pp**"
                if is_high_accuracy
                else (f"{model_acc * 100:+.1f}pp" if model_acc is not None else "—")
            )
            ...
            lines.append(
                f"| {cat_display} | {total_volume} | {settled} | {wr_str} | {exp_wr_str} | {model_str} | {yield_str} | {stake_exposure} | {pnl_str} |"
            )
