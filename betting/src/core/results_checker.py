...
def evaluate_double_chance(dc_part: str, h: int, a: int, home_team: Optional[str] = None, away_team: Optional[str] = None) -> Optional[bool]:
    ...
    if len(choices) == 2:
        ...
        def is_a(c):
            if c in ('2', 'away'): return True
            if not away: return False
            c_clean = re.sub(r'[^a-z0-9]', '', c)
    ...
    if any(x in dc_part for x in ("1x", "x1", "home/x", "x/home", "home or draw", "draw or home", "1/x", "x/1")):
        return h >= a
    if any(x in dc_part for x in ("x2", "2x", "x/away", "away/x", "draw or away", "away or draw", "x/2", "2/x")):
        return a >= h
    if any(x in dc_part for x in ("12", "21", "home/away", "away/home", "home or away", "away or home", "1/2", "2/1")):
        return h != a

    return None


def evaluate_1x2(res_part: str, h: int, a: int, home_team: Optional[str] = None, away_team: Optional[str] = None) -> Optional[bool]:
    res_part = res_part.lower().strip()
    home = home_team.lower().strip() if home_team else None
    away = away_team.lower().strip() if away_team else None
    
    if res_part in ('1', 'home') or (home and (res_part == home or res_part in home or home in res_part)):
        return h > a
    if res_part in ('2', 'away') or (away and (res_part == away or res_part in away or away in res_part)):
        return a > h
...
def evaluate_total(total_part: str, total_goals: int) -> Optional[bool]:
...
def evaluate_btts(btts_part: str, h: int, a: int) -> Optional[bool]:
...
def evaluate_selection(category: str, selection: str, h1: int, a1: int, h2: int, a2: int, home_team: Optional[str] = None, away_team: Optional[str] = None) -> Optional[bool]:
    """
    ...
    if ("1h" in cat or "1st half" in cat) and ("1x2" in cat or "match result" in cat) and "total" in cat:
        parts = sel.split('&')
        if len(parts) == 2:
            p1, p2 = parts[0].strip(), parts[1].strip()
    ...
    if ("2h" in cat or "2nd half" in cat) and ("1x2" in cat or "match result" in cat) and "total" in cat:
        parts = sel.split('&')
        if len(parts) == 2:
            p1, p2 = parts[0].strip(), parts[1].strip()
    ...
    if "1x2 & btts" in cat and "1h" not in cat and "2h" not in cat and "1st half" not in cat and "2nd half" not in cat:
        parts = sel.split('&')
        if len(parts) == 2:
...
def clean_team_name(name: str) -> str:
    ...
    for word in ["fc", "cf", "united", "utd", "city", "town", "rovers", "athletic", "afc", "montevideo", "club"]:
...
async def fetch_sofascore_json(page, url: str) -> Optional[dict]:
    ...
    async with async_playwright() as p:
        ...
        for row in unsettled:
            ...
            try:
                ...
                # Only resolve matches started at least 2.5 hours ago
                if datetime.now(timezone.utc) < dt + timedelta(hours=2.5):
                    print(f"[RESULTS] Skipping {home_team} vs {away_team} — match has not finished yet (started at {start_time})")
                    continue
            except (ValueError, TypeError) as e:
                print(f"[RESULTS] Warning: Could not parse start_time '{start_time}' for {home_team} vs {away_team}: {e}")
                continue

            print(f"[RESULTS] Looking up: {home_team} vs {away_team} (started {start_time})")

            # Formulate the target timestamp and search date range to account for timezone drift
            ...
            # 1. Try checking daily schedules
            ...
            for ev in events:
                ...
                if (check_name_match(home_team, ev_home) and check_name_match(away_team, ev_away)) or \
                   (check_name_match(home_team, ev_away) and check_name_match(away_team, ev_home)):
                    if ev_timestamp:
                        time_diff = abs(ev_timestamp - target_timestamp)
                        if time_diff <= 86400:  # within 24 hours of scheduled kickoff
                            candidates.append((ev, time_diff))

            # 2. Try search API fallback if daily schedule has no matching candidate
            if not candidates:
                print(f"[RESULTS] Match not found in daily schedules, attempting SofaScore search fallback...")
                import urllib.parse
            ...
            if not candidates:
                print(f"[RESULTS] WARNING: No SofaScore match candidate found for {home_team} vs {away_team}.")
                continue

            # Pick the candidate with the closest kickoff time
            candidates.sort(key=lambda x: x[1])
            best_event = candidates[0][0]
            event_id = best_event.get("id")

            # Fetch authoritative match details
            ...
            if not is_finished:
                print(f"[RESULTS] Match is not finished yet. Status: {status_type} ({status.get('description', '')})")
                continue
            ...
            # Graceful fallback for missing period scores on 0-0 draws
            if h1 is None or a1 is None or h2 is None or a2 is None:
                ...
                if ft_h == 0 and ft_a == 0:
                    ...
                    print(f"[RESULTS] Missing period scores for non-zero result. HomeScore: {home_score}, AwayScore: {away_score}")
                    continue

            # Handle possible home/away team swap in the provider's listing
            is_swapped = check_name_match(home_team, ev_detail.get("awayTeam", {}).get("name", ""))
            if is_swapped:
                print(f"[RESULTS] Swapping home/away scores to match database ordering.")
                h1, a1 = a1, h1
                h2, a2 = a2, h2

            print(f"[RESULTS] Resolved score: {home_team} {h1+h2} – {a1+a2} {away_team} (HT {h1}:{a1}, 2H {h2}:{a2})")

            # Settle all unsettled bets for this match
