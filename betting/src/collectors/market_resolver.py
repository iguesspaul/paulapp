    ...
    def resolve(self, market_name, selection_name):
        ...
        if "1st/2nd half both teams to score" in market_name:
            ...
            if len(parts) == 2:
                ...
                for h in range(7):
                    for a in range(7):
                        if (h1_yes and h > 0 and a > 0) or (not h1_yes and (h == 0 or a == 0)):
                            prob1 += self.h1_grid[h][a]

                        if (h2_yes and h > 0 and a > 0) or (not h2_yes and (h == 0 or a == 0)):
                            prob2 += self.h2_grid[h][a]
                return prob1 * prob2
            return 0.0
        ...
        if "1x2" in market_name and "both teams to score" in market_name:
            ...
            is_home = (
                "1 &" in selection_name
                or ("home" in selection_name)
                or (not is_draw and not is_away and "&" in selection_name)
            )
            ...
            for h in range(7):
                for a in range(7):
                    ...
                    match_win = False
                    if (is_home and h > a) or (is_away and a > h) or (is_draw and h == a):
                        match_win = True
        ...
        if "double chance" in market_name and "both teams to score" in market_name:
            ...
            for h in range(7):
                for a in range(7):
                    btts = (h > 0 and a > 0) if is_yes else (h == 0 or a == 0)
                    match_win = False
                    if (dc_1x and h >= a) or (dc_x2 and a >= h) or (dc_12 and h != a):
                        match_win = True
        ...
        if (
            ("both teams to score" in market_name or "btts" in market_name)
            and "1x2" not in market_name
            and "double chance" not in market_name
        ):
        ...
        if "margin" in market_name:
            ...
            if side and margin_match:
                ...
                for h in range(7):
                    for a in range(7):
                        if (side == "home" and (h - a) == n) or (side == "away" and (a - h) == n):
                            prob += active_grid[h][a]
                return prob

        # Draw
        if "draw" in selection_name or selection_name.strip() == "x":
            prob = 0.0
            for h in range(7):
        ...
        if "1x2" in market_name or "match winner" in market_name:
            if selection_name.strip() == "1" or "home" in selection_name:
                prob = 0.0
                for h in range(7):
                    for a in range(7):
                        if h > a:
                            prob += active_grid[h][a]
                return prob
            elif selection_name.strip() == "2" or "away" in selection_name:
                prob = 0.0
                for h in range(7):
                    for a in range(7):
                        if a > h:
