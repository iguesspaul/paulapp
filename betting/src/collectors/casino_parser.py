...
class CasinoParser:
    ...
    def extract_markets(self, file_path):
        ...
        for market in data.get("markets", []):
            ...
            if (
                "score" not in m_lower
                and "ht/ft" not in m_lower
                and "both teams to score" not in m_lower
                and "total" not in m_lower
                and "over/under" not in m_lower
            ):
                continue

            market_data = {"name": market_name, "selections": []}
            ...
            for odd_id in odd_ids:
                ...
                if odd:
                    ...
                    if name and price:
                        market_data["selections"].append({"name": str(name), "price": float(price)})
